...
class WeCLIP(nn.Module):
    def __init__(
        ...
        self.decoder = DecoderTransformer(
            width=self.embedding_dim, layers=3, heads=8, output_dim=self.num_classes
        )

        self.bg_text_features = zeroshot_classifier(
            BACKGROUND_CATEGORY, ["a clean origami {}."], self.encoder
        )
        self.fg_text_features = zeroshot_classifier(
            new_class_names, ["a clean origami {}."], self.encoder
        )
    ...
    def forward(self, img, img_names, mode="train"):
        ...
        fts_all, attn_weight_list = generate_clip_fts(
            img, self.encoder, require_all_fts=True
        )
        ...
        if self.require_all_fts == True:
            cam_fts_all = (
                fts_all_stack[-1].unsqueeze(0).permute(2, 1, 0, 3)
            )  # (1, hw, 1, c)
        else:
        ...
        all_img_tokens = all_img_tokens.reshape(
            -1, b, img_tokens_channel, h // 16, w // 16
        )  # (11, b, c, h, w)
        ...
        for i, img_name in enumerate(img_names):
            img_path = os.path.join(self.dataset_root_path, "SegmentationClassAug", img_name + ".png")  
            img_i = img[i]
            cam_fts = cam_fts_all[i]
            cam_attn = attn_weight_stack[i]
            seg_attn = attn_pred.unsqueeze(0)[:, i, :, :]

            if self.iter_num > 15000 or mode == "val":  # 40000
                require_seg_trans = True
            else:
                require_seg_trans = False

            cam_refined_list, keys, w, h = perform_single_voc_cam(
                img_path,
                img_i,
                cam_fts,
                cam_attn,
                seg_attn,
                self.bg_text_features,
                self.fg_text_features,
                self.grad_cam,
                mode=mode,
            ...
            bg_score = torch.pow(
                1 - torch.max(cams, dim=0, keepdims=True)[0], self.cam_bg_thres
            ).cuda()
